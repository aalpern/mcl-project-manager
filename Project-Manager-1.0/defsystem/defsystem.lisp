...
;;  4 12/8/95  dbl  Changed APPEND-LOGICAL-PNAMES and APPEND-DIRECTORIES:
;;                  Restore use of logical directories broken by the last
;;                  patch, and correct handling of private files in MCL.
;;  3 12/6/95  dbl  Patch COMPONENT-FULL-PATHNAME-I and APPEND-DIRECTORIES
;;                  for MCL.
;;  2 11/16/95 dbl  Don't ignore *COMPILE-VERBOSE* in MCL.
;;                  Ignore args to RUN-UNIX-PROGRAM in MCL.
...
(defun append-directories (absolute-directory relative-directory)
  ...
  (when (or absolute-directory relative-directory)
    (cond 
     ...
     ((or (namestring-probably-logical absolute-directory)
          #+MCL
          (and (null-string absolute-directory)
               (namestring-probably-logical relative-directory)))
  ...
  (let ((abs (or absolute ""))
    ...
    (unless (or (null-string abs) (null-string rel)
		(char= (char abs (1- (length abs)))
		       #\;)
                ;; The DEFSYSTEM manual says (footnote 9) that names 
                ;; in MCL must end with a trailing colon.  I think this
                ;; check belongs here -- otherwise we can't use both
                ;; forms of MCL logical names (logical directories which
                ;; were present in MCL 1.x, and logical pathnames which
                ;; were introduced in MCL 2.x to track CLtL2.
                #+MCL
                (char= (char abs (1- (length abs)))
		       #\:))
...
(defun component-full-pathname-i (component type &optional (version *version*)
  ...
  (let ((pathname
    ...
    #+(or MCL
          (and (and allegro-version>= (version>= 4 1))
	       (not :logical-pathnames-mk)))
    (when (and (pathname-host pathname) 
               #-MCL (logical-pathname-p pathname)
               #+MCL (typep pathname 'logical-pathname))
	...
	(multiple-value-bind (*version-dir* *version-replace*) 
	  ...
	  (let ((*load-verbose* t) ; nil
	    ...
	    (declare (special *compile-verbose* #-MCL *compile-file-verbose*)
		     (ignore #-MCL *compile-verbose* #-MCL *compile-file-verbose*))
...
(defun run-unix-program (program arguments)
  #+:MCL (declare (ignore program arguments))
